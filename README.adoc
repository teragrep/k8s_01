...
Edit `values.yaml` to contain wanted relp target and relp target port, then run `helm install kubelogreader .` to create and deploy an example instance. It uses `etc/config.json` and default `log4j2.xml` for configurations and persistent volumeclaim for statestore.
