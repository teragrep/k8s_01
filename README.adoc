...
Edit `values.yaml` to contain wanted relp target and relp target port, then run `helm template .` to receive minimal example config. It uses `etc/config.json` and default `log4j2.xml` for configurations and persistent volumeclaim for  statestore.
